...
bool checkobstacle()
{
  ...
  if (distance == 0) {
    ...
    MotorState st;
    st.interval=100;
    st.motorstate = ((random(1)==0)?MOVE_LEFT:MOVE_RIGHT);
    state.enqueue(st);
...
void loop() {
  ...
  parseCommand(stateint, controlvalue);
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    if (!state.isEmpty()) 
    {
      MotorState st = state.dequeue();
      motorstate = st.motorstate;
